...
func buildOptions(opts tags.Options, err bool) {
	...
	if opts["tag_only"] != true {
		parts := []string{
			...
			"focus:ring-first-500 focus:border-first-500 flex-1 block w-full rounded-md sm:text-sm border-sixth-300",
		}
...
func divWrapper(opts tags.Options, fn func(opts tags.Options) tags.Body) *tags.Tag {
	...
	if useLabel {
		div.Prepend(tags.New("label", tags.Options{
			"body":  opts["label"],
			"class": "block text-sm font-medium text-sixth-700 mb-1",
		}))
	...
	idiv.Append(tags.New("p", tags.Options{
		"class": "mt-1 text-xs text-third-600mb-2",
